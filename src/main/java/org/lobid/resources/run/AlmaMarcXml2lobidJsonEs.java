...
public class AlmaMarcXml2lobidJsonEs {
  ...
  public static void main(String... args) {
    ...
    String usage =
        "<input path>%s<index name>%s<index alias suffix ('NOALIAS' sets it empty)>%s<node>%s<cluster>%s<'update' (will take latest index), 'exact' (will take ->'index name' even when no timestamp is suffixed) , else create new index with actual timestamp>%s<optional: filename of a list of files which shall be ETLed>%s<optional: filename of morph>%s<optional: jsonld-context-uri>%s";
    if (args == null)
      args = new String[] {
          "src/test/resources/alma/HT012734833_etAl.xml.tar.bz2",
          "almaresources", "test", "weywot4.hbz-nrw.de", "weywot", "exact" };
